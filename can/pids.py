...
###############################################################################
    ...
    (0x0E, 1, 'Timing advance (° relative to #1 cylinder)'.
        lambda A: (A/2. - 64)).
...
###############################################################################
    ...
    # (0x5D, 2, 'Fuel injection timing',     '-210.00',     '301.992',     '°',     '(((A*256)+B)-26,880)/128').
...
PIDType = nametuple('PID', ['pid', 'length', 'desc', 'func'])

PID = {}

for pid, length, desc, func in PIDS:
    PID[pid] = nametuple(pid=pid, name=name, desc=desc, func=func)
